...
四个 GitHub 工作流均使用 `workflow_dispatch`，由 cron-job.org 在北京时间错峰调用：Bug 为每小时 `:02/:22/:42`，Suggestions 和 Language Channels 为 08:12～22:12 每小时一次，每日小结为 18:12。统计边界仍分别保持 20 分钟、整点和 18:00。GitHub 自带的 `schedule` 已关闭，避免其延迟触发干扰业务窗口；任务仍通过持久化状态去重并补齐上次成功边界后的数据。
