...
REPORT_STATE_PATH = Path(__file__).with_name(".suggestions_report_state.json")
LAST_END_ENV_NAME = "SUGGESTIONS_LAST_END_UTC"
...
def load_last_report_end_utc(
    ...
    try:
        ...
        logger.warning("读取本地 suggestions 状态失败: %s", exc)
    ...
    for source, raw_value in candidates:
        try:
            ...
            logger.warning("忽略无效的 suggestions 状态 %s=%r: %s", source, raw_value, exc)
    return None
...
def catch_up_window(
    ...
    *,
    ...
    if last_end_utc is None:
        ...
        logger.warning("没有持久化 suggestions 状态，仅补算最近一个计划窗口")
...
def run_catch_up_safely(cfg: Config) -> None:
    try:
        ...
        if window is None:
            logger.info("suggestions 没有待补齐的统计窗口，本轮不发送")
            return
        ...
        logger.exception("建议频道补漏任务失败，调度器会继续运行")
